...
class TicketsBusinessScreen extends StatefulWidget {
  const TicketsBusinessScreen({Key? key}) : super(key: key);

  @override
  State<TicketsBusinessScreen> createState() => _TicketsBusinessScreenState();
}

class _TicketsBusinessScreenState extends State<TicketsBusinessScreen>
          ...
          ? chat_model.ChatUsers(
              ...
              companyId: chatData.chatUsers!.companyId != null
                  ? chat_model.CompanyId(
                      sId: chatData.chatUsers!.companyId,
            ...
            if (ticket.status == 'follow_up' &&
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  onPressed: () async {
                    // Navigate to chat screen using the same logic as home view
                    ...
                    // ✅ ADD: Initialize socket before opening ticket chat
                    try {
                      final chatService =
                          Provider.of<ChatService>(context, listen: false);
                      await chatService.waitForSocketReady();
                      await chatService.ensureSocketReadyForChatNavigation(
                          ticket.chatId?.sId ?? '');
                      if (kDebugMode) {
                        print(
                            'Socket ready for ticket chat: ${ticket.chatId?.sId}');
                      }
                    } catch (e) {
                      if (kDebugMode) {
                        print(
                            'Socket initialization failed when opening ticket chat: $e');
                      }
                    }
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) => ChatScreen(
                          ...
                          userId: isCompany
                              ? ticket.chatId?.chatUsers?.companyId ?? ''
                              : ticket.chatId?.chatUsers?.customerId ?? '',
                          userRole: isCompany
                              ? 'company'
                              : isSubaccount
                                  ? 'subaccount'
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocBuilder<TicketsBusinessCubit, TicketsBusinessState>(
        builder: (context, state) {
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              leading: IconButton(
                ...
                onPressed: () => Navigator.pushReplacementNamed(
                    context, Routes.contactUsBusinessRoute),
              ),
              title: Text(
            ...
            body: state is TicketsBusinessLoadingState
                ? const Center(child: CircularProgressIndicator())
                : state is TicketsBusinessSuccessState
                    ? TabBarView(
                        controller: _tabController,
                        children: [
                          // Pending Tickets Tab
                    ...
                    : state is TicketsBusinessEmptyState
                        ? Center(
                            child: Text(
                              'No tickets found',
                              style: TextStyle(
                        ...
                        : state is TicketsBusinessErrorState
                            ? Center(
                                child: Text(
                                  state.error,
                                  style: const TextStyle(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  final result = await Navigator.pushNamed(
                      context, Routes.createTicketBusinessRoute);
                  if (result == true) {
                    if (mounted) {
                      context.read<TicketsBusinessCubit>().getTickets();
                    }
                  }
                },
