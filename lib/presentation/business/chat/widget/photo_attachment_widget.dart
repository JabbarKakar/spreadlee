...
class _PhotoAttachmentWidgetState extends State<PhotoAttachmentWidget> {
  ...
  Future<String> _saveMediaToLocalDirectory(String originalPath) async {
  ...
  Future<void> _pickFromGallery() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (image != null) {
        final localPath = await _saveMediaToLocalDirectory(image.path);
        widget.onPhotosSelected([localPath]);
    ...
    try {
      ...
      if (!hasPermission) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Permission to access videos was denied'),
              duration: Duration(seconds: 2),
            ),
          );
        }
      ...
      if (!mounted) return;
      if (video != null) {
        final localPath = await _saveMediaToLocalDirectory(video.path);
        widget.onPhotosSelected([localPath]);
        Navigator.pop(context);
        widget.onClose?.call();
      }
  ...
  Future<void> _pickVideoFromCamera() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
        const  SnackBar(
            content: Text('Error taking video'),
            duration: const Duration(seconds: 2),
          ),
        );
      }
  ...
  Future<void> _pickDocument() async {
    if (!mounted) return;
    try {
      // You may need to request storage permission here
      setState(() => _isLoading = true);
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        final localPath = await _saveMediaToLocalDirectory(file.path!);
        widget.onPhotosSelected([localPath]);
        Navigator.pop(context);
        widget.onClose?.call();
      }
  ...
  Future<void> _pickAudio() async {
    if (!mounted) return;
    try {
      // You may need to request microphone/storage permission here
      setState(() => _isLoading = true);
      final result = await FilePicker.platform.pickFiles(
        type: FileType.audio,
        allowMultiple: false,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
         const SnackBar(
            content: Text('Error picking audio'),
            duration: const Duration(seconds: 2),
          ),
        );
      }
