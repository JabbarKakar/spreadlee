...
class ChatProvider extends ChangeNotifier {
  ...
  // Add initialize method with lazy initialization
  void initialize() {
    ...
    // Initialize socket and other services asynchronously to avoid blocking UI
    _initializeAsync();
  }
  ...
  Future<void> _initializeAsync() async {
    ...
    try {
      // Initialize socket asynchronously to avoid blocking UI
  ...
  void _setupSocketEventListeners() {
    ...
    _chatService.onUserStartedTyping((data) {
      if (_isDisposed) return;
      if (kDebugMode) {
        print('ChatProvider: User started typing: $data');
      }

      final eventChatId = data['chatId'] ?? data['chat_id'];
      final eventUserId = data['userId'] ?? data['user_id'];

      if (eventChatId == chatId && eventUserId != userId) {
        if (kDebugMode) {
          print('ChatProvider: Other user started typing');
        }
        _typingUsers[eventUserId] = true;

        // Force immediate UI update for typing indicator
        if (kDebugMode) {
          print('ChatProvider: Forcing immediate UI update for typing start');
        }
    ...
    _chatService.onUserStoppedTyping((data) {
      if (_isDisposed) return;
      if (kDebugMode) {
        print('ChatProvider: User stopped typing: $data');
      }
      ...
      if (eventChatId == chatId && eventUserId != userId) {
        if (kDebugMode) {
          print('ChatProvider: Other user stopped typing');
        }
        _typingUsers.remove(eventUserId);

        // Force immediate UI update for typing indicator
        if (kDebugMode) {
          print('ChatProvider: Forcing immediate UI update for typing stop');
        }
      ...
      if (chatIdFromEvent == chatId && updatedInvoice != null) {
        ...
        for (int i = 0; i < _messages.length; i++) {
          ...
          if (msgInvoiceId != null && msgInvoiceId == invoiceId) {
            _messages[i] = msg.copyWith(messageInvoice: updatedInvoice);
            updated = true;
          }
        }
        if (updated) {
          notifyListeners();
          if (kDebugMode) {
            print('ChatProvider: Updated invoice in chat messages');
          }
        }
      }
    });
  ...
  bool get isTyping => _isDisposed ? false : _isTyping;
  String? get typingUserId => _isDisposed ? null : _typingUserId;
  ...
  Map<String, bool> get typingUsers =>
      _isDisposed ? {} : Map.unmodifiable(_typingUsers);
  Map<String, DateTime> get typingTimestamps =>
      _isDisposed ? {} : Map.unmodifiable(_typingTimestamps);
  ...
  // New methods for typing indicators and user status
  void startTyping() {
    if (_isDisposed) return;
    if (kDebugMode) {
      print('ChatProvider: Starting typing for chat: $chatId');
    }
  ...
  void stopTyping() {
    if (_isDisposed) return;
    if (kDebugMode) {
      print('ChatProvider: Stopping typing for chat: $chatId');
    }
  ...
  // Method to clean up old typing indicators (call this periodically)
  void cleanupTypingIndicators() {
    if (_isDisposed) return;
    final now = DateTime.now();
    final keysToRemove = <String>[];

    for (var entry in _typingTimestamps.entries) {
    ...
    if (keysToRemove.isNotEmpty) {
      // Force immediate UI update for real-time typing indicators
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!_isDisposed) {
          notifyListeners();
        }
